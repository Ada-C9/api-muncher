  ...
  describe 'search_recipes' do
    ...
    it "gives a list of recipes" do
      VCR.use_cassette("recipe") do
        recipes = MuncherApiWrapper.search_recipes("chicken")
    ...
    it "returns an empty array if there are no recipes matching the serarch term" do
      VCR.use_cassette("recipe") do
        recipes = MuncherApiWrapper.search_recipes("")
        recipes.must_equal []
      end
    end
  ...
  describe 'show_recipe' do
    ...
    it "shows the details for a single recipe" do
      VCR.use_cassette("recipes") do
        recipe = MuncherApiWrapper.show_recipe("7bf4a371c6884d809682a72808da7dc2")
        recipe.wont_be_nil
    ...
    it "throws an error with a bogus id" do
      VCR.use_cassette("recipes") do
        proc { MuncherApiWrapper.show_recipe("").must_raise ArgumentError }
      end
    end
